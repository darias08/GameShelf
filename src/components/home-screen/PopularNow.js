            ...
            renderItem={({ item }) => {
              ...
              if (!item.cover) {
                  return (
                    ...
                    involveCompanies: item.involved_companies, gameGenres: item.genres, artworks: item.artworks, age_Rating: item.age_ratings, item })}>
                ...
                return (
                  <TouchableOpacity activeOpacity={0.7} onPress = {()=>props.navigation.navigate('GamePreview',  
                    { gameName: item.name, gameCover: item.cover.image_id, gameReleased: item.first_release_date, gamePlatforms: item.platforms, 
                    gameSummary: item.summary, SG: item.similar_games, Screenshot: item.screenshots, Videos: item.videos,
                    involveCompanies: item.involved_companies, gameGenres: item.genres, artworks: item.artworks, total_Rating: item.total_rating, age_Rating: item.age_ratings, 
